...
function CurrentProjectID() {
	...
	return "{62B16F30-7B01-4FF2-AFCB-D258EDB35A44}";
}
...
class cField {
	...
	function setFormValue($v, $current = TRUE) {
		...
		if (is_array($v))
			$v = implode(",", $v);
		if ($this->FldDataType == EW_DATATYPE_NUMBER && !is_numeric($v)) // Check data type
			$this->FormValue = NULL;
		else
			$this->FormValue = $v;
		if ($current)
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($validateUser) {
						...
						$this->setSessionUserID($rs->fields('iduser')); // Load User ID
						if (is_null($rs->fields('priv'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('priv'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $UserTableConn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('iduser'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
