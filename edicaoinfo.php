...
//
// Table class for edicao
//
class cedicao extends cTable {
	var $idcontent;
	var $titulo;
	var $imagem;
	var $conteudo;
	...
	function __construct() {
		...
		$this->TableVar = 'edicao';
		$this->TableName = 'edicao';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`edicao`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// idcontent
		$this->idcontent = new cField('edicao', 'edicao', 'x_idcontent', 'idcontent', '`idcontent`', '`idcontent`', 3, -1, FALSE, '`idcontent`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// titulo
		$this->titulo = new cField('edicao', 'edicao', 'x_titulo', 'titulo', '`titulo`', '`titulo`', 200, -1, FALSE, '`titulo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->titulo->Sortable = TRUE; // Allow sort
		$this->fields['titulo'] = &$this->titulo;

		// imagem
		$this->imagem = new cField('edicao', 'edicao', 'x_imagem', 'imagem', '`imagem`', '`imagem`', 205, -1, TRUE, '`imagem`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->imagem->Sortable = TRUE; // Allow sort
		$this->imagem->ImageResize = TRUE;
		$this->fields['imagem'] = &$this->imagem;

		// conteudo
		$this->conteudo = new cField('edicao', 'edicao', 'x_conteudo', 'conteudo', '`conteudo`', '`conteudo`', 201, -1, FALSE, '`conteudo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->conteudo->Sortable = TRUE; // Allow sort
		$this->fields['conteudo'] = &$this->conteudo;

		// aprov
		$this->aprov = new cField('edicao', 'edicao', 'x_aprov', 'aprov', '`aprov`', '`aprov`', 3, -1, FALSE, '`aprov`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->aprov->Sortable = TRUE; // Allow sort
		$this->aprov->OptionCount = 2;
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`edicao`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("edicaoview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("edicaoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "edicaoadd.php?" . $this->UrlParm($parm);
		else
			$url = "edicaoadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("edicaoedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("edicaoadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("edicaodelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->aprov->Exportable) $Doc->ExportCaption($this->aprov);
				} else {
					if ($this->titulo->Exportable) $Doc->ExportCaption($this->titulo);
				}
				$Doc->EndExportRow();
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->aprov->Exportable) $Doc->ExportField($this->aprov);
					} else {
						if ($this->titulo->Exportable) $Doc->ExportField($this->titulo);
					}
					$Doc->EndExportRow();
