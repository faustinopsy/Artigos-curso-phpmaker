...
$conteudo_add = NULL; // Initialize page object first

class cconteudo_add extends cconteudo {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "conteudoview.php")
...
// Create page object
if (!isset($conteudo_add)) $conteudo_add = new cconteudo_add();

// Page init
$conteudo_add->Page_Init();

// Page main
$conteudo_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$conteudo_add->Page_Render();
?>
...
var CurrentForm = fconteudoadd = new ew_Form("fconteudoadd", "add");

// Validate form
fconteudoadd.Validate = function() {
...
fconteudoadd.Form_CustomValidate = 
...
fconteudoadd.ValidateRequired = true;
<?php } else { ?>
fconteudoadd.ValidateRequired = false; 
<?php } ?>
...
<?php $conteudo_add->ShowPageHeader(); ?>
<?php
$conteudo_add->ShowMessage();
?>
<form name="fconteudoadd" id="fconteudoadd" class="<?php echo $conteudo_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($conteudo_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $conteudo_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fconteudoadd", "x_conteudo", 35, 4, <?php echo ($conteudo->conteudo->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $conteudo_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
