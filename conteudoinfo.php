...
//
// Table class for conteudo
//
class cconteudo extends cTable {
	var $idcontent;
	var $titulo;
	var $imagem;
	var $conteudo;
	...
	function __construct() {
		...
		$this->TableVar = 'conteudo';
		$this->TableName = 'conteudo';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`conteudo`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// idcontent
		$this->idcontent = new cField('conteudo', 'conteudo', 'x_idcontent', 'idcontent', '`idcontent`', '`idcontent`', 3, -1, FALSE, '`idcontent`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// titulo
		$this->titulo = new cField('conteudo', 'conteudo', 'x_titulo', 'titulo', '`titulo`', '`titulo`', 200, -1, FALSE, '`titulo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->titulo->Sortable = TRUE; // Allow sort
		$this->fields['titulo'] = &$this->titulo;

		// imagem
		$this->imagem = new cField('conteudo', 'conteudo', 'x_imagem', 'imagem', '`imagem`', '`imagem`', 205, -1, TRUE, '`imagem`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->imagem->Sortable = TRUE; // Allow sort
		$this->imagem->ImageResize = TRUE;
		$this->fields['imagem'] = &$this->imagem;

		// conteudo
		$this->conteudo = new cField('conteudo', 'conteudo', 'x_conteudo', 'conteudo', '`conteudo`', '`conteudo`', 201, -1, FALSE, '`conteudo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->conteudo->Sortable = TRUE; // Allow sort
		$this->fields['conteudo'] = &$this->conteudo;

		// aprov
		$this->aprov = new cField('conteudo', 'conteudo', 'x_aprov', 'aprov', '`aprov`', '`aprov`', 3, -1, FALSE, '`aprov`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'HIDDEN');
		...
		// visual
		$this->visual = new cField('conteudo', 'conteudo', 'x_visual', 'visual', '`visual`', '`visual`', 3, -1, FALSE, '`visual`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// user
		$this->user = new cField('conteudo', 'conteudo', 'x_user', 'user', '`user`', '`user`', 3, -1, FALSE, '`user`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`conteudo`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("conteudoview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("conteudoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "conteudoadd.php?" . $this->UrlParm($parm);
		else
			$url = "conteudoadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("conteudoedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("conteudoadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("conteudodelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->idcontent->Exportable) $Doc->ExportCaption($this->idcontent);
					if ($this->titulo->Exportable) $Doc->ExportCaption($this->titulo);
				}
				$Doc->EndExportRow();
			}
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->idcontent->Exportable) $Doc->ExportField($this->idcontent);
						if ($this->titulo->Exportable) $Doc->ExportField($this->titulo);
					}
					$Doc->EndExportRow();
				}
	...
	function GetUserIDSubquery(&$fld, &$masterfld) {
		...
		$sWrk = "";
		$sSql = "SELECT " . $masterfld->FldExpression . " FROM `conteudo`";
