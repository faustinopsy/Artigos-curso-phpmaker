...
//$showname = ew_ExecuteScalar("SELECT (email) FROM usuarios WHERE email= '" . ew_AdjustSql(CurrentUserName()) . "'");
$showname = ew_ExecuteScalar("SELECT (email) FROM usuarios  WHERE email='" . ew_AdjustSql(CurrentUserName()) . "'");
}
$Menu->AddMenuItem(10000, "mi_username", "Bem vindo !<br> ".$showname, ".", -2, "", IsLoggedIn());
$Menu->MoveItem("mi_username", $Menu->Count() + 10); // Move to last 
}
}
